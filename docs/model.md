...
2. In each snapshot, the **extractor adapters** read their assigned files. For every named
   ...
   belongs to one **part**. The parts are the **type**, **body**, and **docs**.
...
| type              | the part callers can see, including the name. `Occurrence::type_from_compiler` is the compiler's own view of it and, when present in both snapshots, decides whether callers broke instead of the written text. `Edits::type_changed` says it changed.                               |
...
| occurrence        | a definition as it exists in one snapshot (`Occurrence`): locator, role, parent, kind, file, parts, type from the compiler.                                                                                                                                                          |
